...
class DatabaseHandler(val context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, 1) {
    ...
    fun readOfflineMenuData() : MutableList<MenuItem> {
        ...
        if (result.moveToFirst()) {
            do {
                val item = MenuItem()
                item.imageUrl = result.getString(result.getColumnIndex(COL_ITEM_IMAGE_URL)).toString()
                ...
                item.itemShortDesc = result.getString(result.getColumnIndex(COL_ITEM_DESC)).toString()
    ...
    fun readCurrentOrdersData() : MutableList<CurrentOrderItem> {
        ...
        if (result.moveToFirst()) {
            do {
                ...
                item.takeAwayTime = result.getString(result.getColumnIndex(COL_CURRENT_ORDER_TAKE_AWAY_TIME))
                item.paymentStatus = result.getString(result.getColumnIndex(COL_CURRENT_ORDER_PAYMENT_STATUS))
                item.orderItemNames = result.getString(result.getColumnIndex(COL_CURRENT_ORDER_ITEM_NAMES))
                item.orderItemQuantities = result.getString(result.getColumnIndex(COL_CURRENT_ORDER_ITEM_QUANTITIES))
                item.totalItemPrice = result.getString(result.getColumnIndex(COL_CURRENT_ORDER_TOTAL_ITEM_PRICE))
    ...
    fun deleteCurrentOrderRecord(orderId: String): String {
        ...
        val updateOrderHistoryQuery = "UPDATE $ORDER_HISTORY_TABLE_NAME SET $COL_ORDER_STATUS='Order Cancelled' WHERE $COL_ORDER_ID='$orderId';"
    ...
    fun insertSavedCardDetails(item: SavedCardItem): Int {
        ...
        val query = "SELECT * FROM $SAVED_CARDS_TABLE_NAME WHERE $COL_SAVED_CARD_NUMBER='${item.cardNumber}';"
    ...
    fun readSavedCardsData() : MutableList<SavedCardItem> {
        ...
        if (result.moveToFirst()) {
            do {
                ...
                item.cardHolderName = result.getString(result.getColumnIndex(COL_SAVED_CARD_HOLDER_NAME))
                item.cardExpiryDate = result.getString(result.getColumnIndex(COL_SAVED_CARD_EXPIRY_DATE))
                list.add(item)
            } while (result.moveToNext())
        }
